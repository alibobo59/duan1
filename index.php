    ...
    switch ($act) {
        ...
        case 'delcart':
            if (isset($_GET['idcart'])) {
                $idcart = $_GET['idcart']; // Lấy chỉ số của sản phẩm cần xóa
                unset($_SESSION['mycart'][$idcart]); // Xóa sản phẩm dựa trên chỉ số
                $_SESSION['mycart'] = array_values($_SESSION['mycart']); // Sắp xếp lại chỉ số mảng
            } else {
                $_SESSION['mycart'] = []; // Xóa toàn bộ giỏ hàng nếu không có idcart
            }
            header('Location: index.php?act=viewcart'); // Điều hướng lại trang giỏ hàng
            break;
        case 'bill':
